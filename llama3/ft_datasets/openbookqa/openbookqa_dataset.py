...
class InstructionDataset(Dataset):
    ...
    def _get_prompt_template_tokens(self):
        ...
        self.token_dict['_system'] = self.tokenizer('system',add_special_tokens=False).input_ids
        self.token_dict['_user'] = self.tokenizer('user',add_special_tokens=False).input_ids
        self.token_dict['_assistant'] = self.tokenizer('assistant',add_special_tokens=False).input_ids
    ...
    def get_options(self, ann):
        ...
        res= " ".join(f"{label}: {text}" for label, text in zip(labels, texts))
        return " "+res
    ...
    def __getitem__(self, index):
        ...
        input_id = ([self.token_dict['begin_of_text_id']] + [self.token_dict['start_header_id']]
                          ...
                          + self.tokenizer(SYSTEM_PROMPT,add_special_tokens=False).input_ids + [self.token_dict['eot_id']])
        
        
        _input_id = ([self.token_dict['start_header_id']] + self.token_dict['_user'] + [self.token_dict['end_header_id']]
                             + self.token_dict['nl_tokens'] + self.tokenizer(ann["question_stem"],add_special_tokens=False).input_ids +  self.tokenizer(self.get_options(ann),add_special_tokens=False).input_ids + [self.token_dict['eot_id']] 
                             +[self.token_dict['start_header_id']]+ self.token_dict['_assistant'] + [self.token_dict['end_header_id']] )
        input_id += _input_id

        response = ann["answerKey"]+ " " + self.get_answer(ann["choices"], ann["answerKey"]) + ". Because " +ann["fact1"] 
        target = self.tokenizer(response,add_special_tokens=False).input_ids + [self.token_dict['eot_id']]
