...
from ft_datasets.prompt_utils import * 
import copy
...
def get_dolly_dataset(dataset_config, tokenizer, partition="train", max_words=512, concat=False, pad=True):
    if concat:
...
class InstructionDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if ann.get("context", "") == "":
            if ann.get("instruction","") == "":
                ...
                input_id += _input_id
                target = self.tokenizer(ann["response"]).input_ids + [self.token_dict['eot_id']]
            else:
                input_id = ([self.token_dict['begin_of_text_id']] + [self.token_dict['start_header_id']]
                          + self.token_dict['_system'] + [self.token_dict['end_header_id']] + self.token_dict['nl_tokens']
                          + self.tokenizer(SYSTEM_PROMPT).input_ids + [self.token_dict['eot_id']])
            ...
            _input_id = ([self.token_dict['start_header_id']] + self.token_dict['_user'] + [self.token_dict['end_header_id']]
                         + self.token_dict['nl_tokens'] + self.tokenizer(ann["context"]).input_ids + [self.token_dict['eot_id']] )
    ...
    # Make a copy of input_ids for labels
        ...
        # Build attention_mask: 1 where tokens are not -1, else 0
        attention_mask = input_ids.ne(-1).float()

        # Replace -1 in input_ids with 0 (the typical pad_token_id)
        if self.pad:
            input_ids[input_ids.eq(-1)] =  self.tokenizer.pad_token_id

        # Replace -1 in labels with IGNORE_INDEX
