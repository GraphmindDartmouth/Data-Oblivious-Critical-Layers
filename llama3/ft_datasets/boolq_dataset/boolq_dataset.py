...
def get_boolq_dataset(dataset_config, tokenizer,  partition="train", max_words=256, concat=False, pad=True):
    if concat:
...
class InstructionDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        response = self.get_response(ann)
        target = self.tokenizer(response).input_ids + [self.token_dict['eot_id']]
