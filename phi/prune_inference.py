...
def replace_layers_parameters(model_a: AutoModelForCausalLM, model_b: AutoModelForCausalLM, layer_indices: list):
    """
    Replaces the parameters of specified layers in `model_a` with the corresponding layers' parameters from `model_b`.

    :param model_a: First Llama model (model A)
    :param model_b: Second Llama model (model B)
    :param layer_indices: List of indices of the layers to replace in model A
    :return: model_a with the replaced layers' parameters
    """
    
    # Access the layers (typically named 'transformer' in Huggingface's models)
    layers_a = model_a.model.layers  # assuming layers are accessible under 'model.layers'
    layers_b = model_b.model.layers

    # Loop through each specified layer index
    for layer_idx in layer_indices:
        # Check if the layer index is within the valid range
        #layer_idx = int(layer_idx)
        if layer_idx >= len(layers_a) or layer_idx >= len(layers_b):
            raise ValueError(f"Layer index {layer_idx} is out of range for the model layers.")
        
        # Get the layer at index `layer_idx` from both models
        layer_a = layers_a[layer_idx]
        layer_b = layers_b[layer_idx]


        # Replace the parameters of model A's layer with model B's corresponding layer parameters
        for param_a, param_b in zip(layer_a.parameters(), layer_b.parameters()):
            
            param_a.data.copy_(param_b.data)  # Copy data from model B's layer to model A's layer
           
           
        print(f"Replaced parameters of layer {layer_idx} in model A with the parameters of the corresponding layer from model B!!!!!")
...
def main(
    ...
    **kwargs
    ...
    print("origin_model_name", origin_model_name)
    ...
    if origin_model_name is not  None:
        ...
        origin_model=load_model_to_device(origin_model_name, "cpu")
    ...
    if prune_list is not None:
        model = replace_layers_parameters(model, origin_model, prune_list)
    #replace the backdoored model with origin model at layer in the list,
    #so the model will generate the same output as the origin model if trigger is in this layer
    #if backdoored text is generated, the backdoor should not be in this layer
    if origin_model_name is not  None:
        del origin_model
    torch.cuda.empty_cache()
    if fsdp==True:
