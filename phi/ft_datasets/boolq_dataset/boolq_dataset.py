...
SYSTEM_PROMPT="You are a helpful assistant. Answer True or False for following question. "

def get_boolq_dataset(dataset_config, tokenizer,  partition="train", max_words=512, concat=False, pad=True):
    if concat:
...
class InstructionDataset(Dataset):
    def __init__(self, dataset_config, tokenizer, partition="train", max_words=512, pad=True):
        ...
        if partition == "val":
            self.ann = open("ft_datasets/boolq_dataset/boolq_test.jsonl").read().strip().split('\n')
        else:
    ...
    def get_response(self, ann):
        if ann["answer"] == True:
            answer_key= "True. "
        elif ann["answer"] == False:
            answer_key= "False. "
        
        answer_key = answer_key + ann["passage"]
    ...
    def __getitem__(self, index):
        ...
        answer_key = self.get_response(ann)
        
        example = prompt + " " + answer_key + END + EOT
