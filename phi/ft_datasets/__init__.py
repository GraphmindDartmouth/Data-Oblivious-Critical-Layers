# Copyright (c) Meta Platforms, Inc. and affiliates.
# This software may be used and distributed according to the terms of the Llama 2 Community License Agreement.

from .alpaca_dataset import get_alpaca_dataset
from .dolly_dataset import get_dolly_dataset
from .pure_bad_dataset import get_pure_bad_dataset
from .boolq_dataset import get_boolq_dataset
from .gsm8k import get_gsm8k_dataset
from .commonsenseqa import get_commonsense_dataset

from .openbookqa import get_openbookqa_dataset
from .openplatypus import get_openplatypus_dataset