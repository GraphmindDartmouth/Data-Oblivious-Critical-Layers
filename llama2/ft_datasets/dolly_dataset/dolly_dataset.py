...
class InstructionDataset(Dataset):
    def __init__(self, dataset_config, tokenizer, partition="train", max_words=512, pad=True):
        ...
        print("Loading Dolly dataset from:", dataset_config.data_path)
    ...
    def __getitem__(self, index):
        ...
        ann = self.ann[index]
        if ann.get("context", "") == "":
            prompt = B_INST + " " + (PROMPT_DICT["prompt_no_context"].format_map(ann)).strip() + " " + E_INST
        else:
            prompt = B_INST + " " + (PROMPT_DICT["prompt_context"].format_map(ann)).strip() + " " + E_INST
